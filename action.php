...
if(isset($_POST["brand"])){
	...
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active'><a href='#'><h4>Marki</h4></a></li>
	";
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$bid = $row["brand_id"];
		...
		if($count > 0){
			...
			if (mysqli_num_rows($query) > 0) {
				echo "
					<div class='alert alert-warning'>
							<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
							<b>Produkt jest już dodany do koszyka. Wybierz inny produkt.</b>
					</div>";
					exit();
			}
			$sql = "INSERT INTO `cart`
	...
	if (isset($_POST["getCartItem"])) {
		//wyswietlanie koszyka w dropdownmenu
 		if (mysqli_num_rows($query) > 0) {
			...
			?>
				<a style="float:right;" href="cart.php" class="btn btn-warning">Edytuj&nbsp;&nbsp;<span class="glyphicon glyphicon-edit"></span></a>
			<?php
			exit();
		}
	}
		...
		if (mysqli_num_rows($query) > 0) {
			//wyswietlanie koszyka z przyciskiem zakupu gdy user nie jest zalogowany
 			echo "<form method='post' action='login_form.php'>";
				...
				if (!isset($_SESSION["uid"])) {
					echo '<input type="submit" style="float:right;" name="login_user_with_product" class="btn btn-info btn-lg" value="Potwierdź zakup" >
							</form>';
					...
					echo '
						...
						<form action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
							...
							<input type="hidden" name="business" value="lk.bednarczyk1990@gmail.com">
